...
async fn initialize_user(transaction: &Transaction<'_>, user_name: &str, password: &str) -> Result<UserDto, Box<dyn std::error::Error>> {
    ...
    let insert_file_root = "insert into file_nodes (id, user_id, title, parent_id, node_type, filesystem_path) values (0, $1, 'ROOT', null, 0, $2)";
    ...
    let user_files_root = format!("/mnt/storage/files/{}", user_id);
    transaction.execute(insert_file_root, &[&user_id, &user_files_root]).await?;
